function JCompressor(_this, option) {
    ...
    if (
        opts.PreviewContainer != undefined &&
        typeof opts.PreviewContainer == "string"
    ) {
    ...
    function ProcessingFn(file) {
        if (file.size > opts.MaxSize) {
            FileEmpty(function () {
                alert(
                    "上传文件大小不能超过" + opts.MaxSize + "B，请重新上传！"
                );
            });
            return false;
        }
        ...
        if (!/(jp|jpe|pn)g|bmp$/.test(file.name.toLowerCase())) {
            FileEmpty(function () {
                alert("图片格式错误，请重新上传！");
            });
            return false;
            ...
            img.onload = function () {
                ...
                CanvasDraw(img, cvsw, cvsh, mimeType, function (imgblob) {
                    if (
                        opts.AllowPngToJpg &&
                        mimeType == "image/png" &&
                        imgblob.size > opts.MaxCompressSize
                    ) {
                        CanvasDraw(
                            img,
                            cvsw,
