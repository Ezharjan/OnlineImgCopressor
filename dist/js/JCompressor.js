function JCompressor(_this, option) {
    ...
    var defaultopts = { MaxSize: 50 * 1024 * 1024, IsImg: true, IsPreview: true, PreviewContainer: (_this.name || _this.id) + "_img", IsCompress: true, MaxWidth: 1080, MaxHeight: 1920, MaxCompressSize: 0, Quality: 0.8, AllowPngToJpg: false, FileType: 'blob', ChangeBefore: function () { }, ChangeAfter: function () { }, FileLoad: function (data, obj) { }, ErrorEmpty: function () { }, ImgPreview: function (data) { ImgPreview(data); }, NeedDownload: false, DownloadLoad: function (nextfn) { } }
    ...
    function ProcessingFn(file) {
        if (file.size > opts.MaxSize) { FileEmpty(function () { alert('上传文件大小不能超过' + opts.MaxSize + 'B，请重新上传！'); }); return false; }
        if (!opts.IsImg) { return false; }
        if (!/(jp|jpe|pn)g|bmp$/.test(file.name.toLowerCase())) { FileEmpty(function () { alert('图片格式错误，请重新上传！'); }); return false; }
    ...
    function FileReady(filedata) {
        if (isMultiple) { fileArr.push(filedata); BlobToBase64(filedata, function (result) { fileBase64Arr.push(result); if (fileBase64Arr.length == files.length) { fileLoad(fileArr, fileBase64Arr); } }); } else { BlobToBase64(filedata, function (result) { fileLoad(filedata, result); }); }
        function fileLoad(filedata, filebase64) {
            if (opts.IsPreview) { openobj.ImgPreview(filebase64); }
    ...
    function ImgPreview(filedata) {
        if (!opts.PreviewContainer) { alert("预览图片元素未找到！"); return; }
        var isimgtag = opts.PreviewContainer.localName == "img"; if (isMultiple) {
            if (isimgtag) { alert("多张图片预览时PreviewContainer属性值不能是img标签"); return; }
            opts.PreviewContainer.innerHTML = ""; filedata.forEach(function (item) { var img = document.createElement("img"); img.setAttribute("src", item); opts.PreviewContainer.appendChild(img); })
        } else { if (isimgtag) { opts.PreviewContainer.src = filedata; } else { alert("单张图片预览时PreviewContainer属性值必须传入img标签"); } }
    }
    function CompressorFn(imgfile) {
        BlobToBase64(imgfile, function (result) {
            var img = new Image(); img.src = result; img.onload = function () {
                var imgw = this.width; var imgh = this.height; var cvsw, cvsh; cvsw = imgw > opts.MaxWidth ? opts.MaxWidth : imgw; cvsh = imgh > opts.MaxHeight ? opts.MaxHeight : imgh; var wp = imgw / opts.MaxWidth; var hp = imgh / opts.MaxHeight; if (wp > hp) { cvsh = imgh / imgw * cvsw; } else if (wp < hp) { cvsw = imgw / imgh * cvsh; }
    ...
    function Base64ToBlob(base64, mimeType) {
        var bytes = window.atob(base64.split(",")[1]); var ab = new ArrayBuffer(bytes.length); var ia = new Uint8Array(ab); for (var i = 0; i < bytes.length; i++) { ia[i] = bytes.charCodeAt(i); }
        var _blob = new Blob([ab], { type: mimeType }); return _blob;
    }
    function FileEmpty(backfn) {
        _this.value = ''; if (opts.IsPreview && opts.PreviewContainer) { if (isMultiple) { opts.PreviewContainer.innerHTML = ''; } else { opts.PreviewContainer.src = ''; } }
        backfn(); openobj.ErrorEmpty(); openobj.ChangeAfter();
    }
    function DownloadLoad(filedata) {
        var isArray = filedata instanceof Array; if (isArray && filedata.length == 1) { filedata = filedata[0]; isArray = false; }
    ...
    function PackInZip(filearr, callback) {
        var zip = new JSZip(); var img = zip.folder("images"); var fileNum = 0; for (var i = 0; i < filearr.length; i++) { (function () { var filei = filearr[i]; BlobToBase64(filei, function (base64data) { base64data = base64data.replace(/(^\s*)data\:.+base64,/, ""); img.file(filei.name, base64data, { base64: true }); fileNum++; if (fileNum == filearr.length) { toBase64Load(); } }) })() }
